...
def random_forest(dataset):
	...
	rf = RandomForestClassifier(max_features = 'sqrt', n_estimators = 600).fit(X_train, y_train)
	rf_predictions = rf.predict(X_test)

	#print(np.equal(rf_predictions, y_test))
	#model accuracy for X_test
...
def random_forest_grid(dataset):
	seed = 7
	np.random.seed(seed)
	...
	parameters = { 'n_estimators': [10, 50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000], 'max_features': ['auto', 'sqrt', 'log2']}
	#parameters = {'kernel': ('linear', 'rbf', 'poly'), 'C':[0.00001, 0.0001, 0.001, 0.01, 0.1, 1, 10, 100, 1000, 10000, 100000], 'gamma':[0.00001, 0.0001, 0.001, 0.01, 0.1, 1, 10, 100]}
